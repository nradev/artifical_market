...
for i in range(1000):
    model.step()
    if not i % 10:
        print("p: {}\td: {}\tW: {}".format(round(model.stock.price,2), round(model.stock.dividend,2),
                                           int(model.global_wealth)))
        print(int(min([agent.cash for agent in model.schedule.agents])))
